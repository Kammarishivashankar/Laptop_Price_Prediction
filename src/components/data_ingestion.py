...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        logging.info('Initiated the data ingestion using the initiate_data_ingestion')
        try:
            df = pd.read_csv('notebooks\\data\\cleaned_data.csv')

            logging.info('Exported the data as pandas dataframe')

            # os.mkdir(os.path.dirname(self.ingestionConfig.train_data_path),exist_ok=True)
...
if __name__ == '__main__':
    ...
    x_train,y_train,x_test,y_test = data_trasformer.initiate_data_transformation(train_data,test_data)

    model_trainer = ModelTrainer()
    print(model_trainer.initiate_model_training(x_train,y_train,x_test,y_test))
